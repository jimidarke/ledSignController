...
class BETABRITE : public HardwareSerial  {
  public:
    //BETABRITE ( uint8_t receivePin, uint8_t transmitPin, const char Type=BB_ST_ALL, const char Address[2]=NULL );
    BETABRITE(uint8_t uart_num, uint8_t receivePin, uint8_t transmitPin, const char Type = BB_ST_ALL, const char Address[2] = NULL);
    ~BETABRITE ( void );
